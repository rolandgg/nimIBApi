import utils, ibContractTypes, ibOrderTypes, ibExecutionTypes,
    ibMarketDataTypes, ibEnums, ibTickTypes, ibFundamentalDataTypes
...
proc handle*[T](payload: seq[string]): T =
  ...
  when T is OrderTracker:
    ...
    if order.hedgeType != HedgeType.Unset and order.hedgeType !=
        HedgeType.Undefined:
  ...
  when T is ContractDescriptionList:
    ...
    for i in 0..nContractDescriptions - 1:
      ...
      if nDerivativeSecTypes > 0:
        ...
        result[i] = ContractDescription(contract: contract,
            derivativeSecTypesList: derivativeSecTypesList)
      else:
        result[i] = ContractDescription(contract: contract,
            derivativeSecTypesList: @[])
