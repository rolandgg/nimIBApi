...
proc parseLiquidHours*(contract: ContractDetails): seq[tuple[
    marketOpen: Datetime, marketClose: Datetime]] =
  var tz: Timezone
  ...
  for day in liquidHours:
    ...
    if openClose.len == 2:
      result.add((marketOpen: parse(openClose[0], "yyyyMMdd:hhmm", tz),
          marketClose: parse(openClose[1], "yyyyMMdd:hhmm", tz)))
