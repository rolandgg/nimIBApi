...
type
  ...
  StatementType = enum
    ibIncomeStatement = "INC", ibBalanceSheet = "BAL",
        ibCashflowStatement = "CAS"
  FilingType = enum
  ...
  FundamentalReport* = object
    ...
    of FinStatements:
      ...
      coaType*: string
      codeMap: Table[string, tuple[statementType: StatementType,
          itemName: string]]
...
proc initStatement(periodEnd: DateTime, periodType: string,
    periodLength: int): Statement =
...
proc `[]`(report: FundamentalReport, key: string, lag: int,
    filingType: FilingType = ibQuarter): Option[float] =
...
proc `[]`(report: FundamentalReport, key: string,
    filingType: FilingType = ibQuarter): Option[float] {.inline.} =
    ...
    if x.kind == xmlElementOpen:
      if x.elementName == "MarketDataItem":
        ...
        if x.kind == xmlCharData:
          case id:
            of "CLPRICE":
              report.closePrice = some((value: parseFloat(x.charData),
                  unit: unit, currency: currency))
            of "MARKETCAP":
              report.marketCap = some((value: parseFloat(x.charData),
                  unit: unit, currency: currency))
            of "52WKHIGH":
              report.high52week = some((value: parseFloat(x.charData),
                  unit: unit, currency: currency))
            of "52WKLOW":
              report.low52week = some((value: parseFloat(x.charData),
                  unit: unit, currency: currency))
    if x.kind == xmlElementEnd:
  ...
  while true:
    ...
    if x.kind == xmlElementOpen:
      ...
      if x.elementName == "mapItem":
        ...
        x.next()
        if x.kind == xmlCharData:
          report.codeMap[key] = (statementType: parseEnum[StatementType](
              statementType), itemName: x.charData)
        continue
...
proc isCumulative(report: FundamentalReport,
    statementType: StatementType): bool {.inline.} =
  case statementType:
  of ibIncomeStatement:
    if report.incomeStatementsQ[0].periodLength != report.incomeStatementsQ[
        1].periodLength:
      return true
  of ibCashflowStatement:
    if report.cashflowStatementsQ[0].periodLength != report.cashflowStatementsQ[
        1].periodLength:
      return true
  else:
    return false
